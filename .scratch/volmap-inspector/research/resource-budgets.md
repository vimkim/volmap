...
Ticket 17 cannot yet set evidence-backed numeric defaults for resident memory, spill capacity, worker count, OOS traversal steps, decoded bytes, scan throughput, or query latency. The implementation has advanced since the initial research pass: it now has snapshot discovery, a deterministic fast envelope scan, immutable revisions, explicit file-allocation traversal, cancellation checks, selective page decoding, bounded OOS traversal, an exact 16-byte canonical fast-fact encoding, automatic private spill, bounded parallel envelope reads with canonical merging, and resident accounting that terminates corruption-heavy diagnostic growth explicitly. It still lacks the cursor query engine, large/dense/corrupt benchmark corpora, and the complete corruption/TDE matrix. Consequently the full workloads whose budgets govern version one still cannot run.

The exact-commit generator and immutable 31-page acceptance corpus now provide a reproducible small functional profile. The retained 192 MiB source snapshot is still insufficient as a benchmark matrix: it does not supply the required medium, large, sparse, highly allocated, or corruption-heavy cases. Its geometry is useful corroborating metadata, not a measured basis for defaults.
