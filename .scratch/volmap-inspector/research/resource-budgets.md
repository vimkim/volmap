...
Ticket 17 cannot yet set evidence-backed numeric defaults for resident memory, spill capacity, worker count, OOS traversal steps, decoded bytes, scan throughput, or query latency. The implementation has advanced since the initial research pass: it now has snapshot discovery, a deterministic fast envelope scan, immutable revisions, explicit file-allocation traversal, cancellation checks, selective page decoding, bounded OOS traversal, an exact 16-byte canonical fast-fact encoding, and automatic private spill. It still lacks the worker pool, cursor query engine, large/dense/corrupt benchmark corpora, and the complete corruption/TDE matrix. Consequently the full workloads whose budgets govern version one still cannot run.
