...
The checkpoint release ELF is 2,016,416 bytes and is reported by `file` as static PIE and by `ldd` as statically linked. This validates the current build shape, not reproducibility or the distribution matrix. Real explicit-file traversal successfully reconciled tracker, heap, E-hash, and heap-reuse headers; `map file:0:128` classified three allocated pages in one sector. Real OOS head `oos:1:2243:0` validated two chunks and 30,728 payload bytes without retaining payload content.
