...
- The initial audit found only hand-built fixtures and two stale mutable external hashes. That gap is now partially closed by `fixtures/e1e651de/`: 31 complete physical pages generated from the exact pinned engine, with a machine-readable hash/role manifest, generator SQL, source-volume hashes, and six direct decoder tests. It covers FTAB/tracker, generic slotted pages, heap header/data, OOS stats and single/multi-chunk pages, B-tree root/leaf, E-hash structural view, catalog, overflow, vacuum, dropped-file, volume, and bitmap families.
- The committed corpus does not yet supply every accepted boundary: exact OOS page/chunk boundaries, generated relocation/NEWHOME, B-tree non-leaf/OID-overflow, recognized opaque QRESULT/AREA/LOG envelopes, or engine-generated AES/ARIA snapshots remain fixture work. Existing deterministic malformed unit tests and synthetic encrypted tests remain valid implementation evidence but do not substitute for those missing generated cases. Recovered artifacts remain excluded from tests.
