...
`--listen` defaults to `127.0.0.1:0`. Version one accepts a numeric IP socket address rather than resolving a hostname. It completes `Inspection::open`, publishes revision zero, and only then binds; a failure before publication never leaves a listener behind. When `--external-origin` is omitted for a concrete address, Volmap derives `http://<listen-ip>:<selected-port>` after binding. The foreground process reports that safe origin without placing a credential in it. An explicit external origin may also describe an SSH-forwarded local port that differs from the remote listener.
