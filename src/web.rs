...
function showSector(sector,historyMode='push'){mapObserver.disconnect();currentSector=sector;currentPage=null;selectedPage=null;selectedSlot=null;renderBreadcrumb('sector');const content=$('workspaceContent'),title=document.createElement('div'),focus=document.createElement('section'),grid=document.createElement('div');title.className='workspace-title';title.innerHTML=`<div><h1>Sector ${sector.sector_id}</h1><p>64 physical pages · select a page to enlarge</p></div>`;focus.className='sector-focus';grid.className='sector-focus-grid';grid.setAttribute('role','grid');grid.setAttribute('aria-label',`Sector ${sector.sector_id}, 64 physical pages`);sector.pages.forEach((page,index)=>{const finding=page.diagnostic.state==='known',node=button('',()=>showPage(page.page_id),`page focus-page ${page.allocation}${finding?' finding':''}${page.page_id===selectedPage?' selected':''}`),kind=document.createElement('span'),identity=document.createElement('span');kind.className='page-kind';kind.textContent=page.page_type.state==='known'?page.page_type.value:'not inspected';identity.className='page-id';identity.textContent=String(page.page_id);node.append(kind,identity);node.setAttribute('role','gridcell');node.setAttribute('aria-label',`Page ${page.page_id}, ${page.allocation}${finding?', finding':''}`);node.onkeydown=event=>moveSectorGrid(event,grid,index);grid.append(node)});focus.append(grid);content.replaceChildren(title,focus);syncBrowserRoute('sector',historyMode)}
