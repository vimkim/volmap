...
async function selectVolume(volume){currentVolume=volume;currentSector=null;selectedPage=null;invalidateVolumeView();document.querySelectorAll('#volumes .nav').forEach(node=>node.classList.toggle('active',node.dataset.volume===String(volume.vol_id)));await showVolume()}
function renderBreadcrumb(level){currentLevel=level;const root=$('drillBreadcrumb');root.replaceChildren();if(level!=='volume'){const back=button('← Back',level==='page'?()=>showSector(currentSector):()=>showVolume(),'back');root.append(back)}const volume=button(`Volume ${currentVolume.vol_id}`,()=>showVolume());root.append(volume);if(level==='sector'||level==='page'){root.append('›');root.append(button(`Sector ${currentSector.sector_id}`,()=>showSector(currentSector)))}if(level==='page'){root.append('›');const page=document.createElement('span');page.textContent=`Page ${selectedPage}`;root.append(page)}}
function createVolumeView(){sectorCursor=null;loadedSectors=0;const generation=++loadGeneration,root=document.createElement('section'),title=document.createElement('div'),map=document.createElement('div'),status=document.createElement('p');root.className='volume-view';title.className='workspace-title';title.innerHTML=`<div><h1>Volume ${currentVolume.vol_id} · full map</h1><p>${currentVolume.total_sectors} sectors · 64 pages per sector · revision ${session.snapshot.revision}</p></div><div id='legend' aria-label='Page allocation legend'><span><i class='swatch unreserved'></i>Unreserved</span><span><i class='swatch reserved-unallocated'></i>Reserved, unallocated</span><span><i class='swatch allocated'></i>Allocated</span><span><i class='swatch system-metadata'></i>System metadata</span><span><i class='swatch finding'></i>Finding outline</span></div>`;map.id='volumeMap';map.setAttribute('aria-label','Full volume sector map');status.id='mapStatus';status.setAttribute('role','status');status.textContent='Loading sector maps…';root.append(title,map,status);volumeView=root;return generation}
async function showVolume(){mapObserver.disconnect();currentSector=null;selectedPage=null;renderBreadcrumb('volume');let generation=loadGeneration;if(!volumeView)generation=createVolumeView();$('workspaceContent').replaceChildren(volumeView);if(sectorCursor===null&&loadedSectors===0)await loadSectorBatch(generation);observeMapEnd()}
...
function appendSector(sector){if(sector.pages.length!==64)throw new Error(`sector ${sector.sector_id} did not contain 64 pages`);sectorCache.set(sector.sector_id,sector);const card=button('',()=>showSector(sector),'sector-card');card.id=`sector-${sector.sector_id}`;card.setAttribute('aria-label',`Sector ${sector.sector_id}, ${sector.reserved?'reserved':'unreserved'}, 64 pages`);const heading=document.createElement('span');heading.className='sector-heading';const title=document.createElement('strong');title.textContent=`Sector ${sector.sector_id}`;const state=document.createElement('span');state.textContent=sector.reserved?'reserved':'unreserved';heading.append(title,state);const pages=document.createElement('span');pages.className='sector-preview-pages';for(const page of sector.pages){const finding=page.diagnostic.state==='known',node=document.createElement('i');node.className=`page preview-page ${page.allocation}${finding?' finding':''}`;pages.append(node)}card.append(heading,pages);volumeView.querySelector('#volumeMap').append(card)}
...
function showSector(sector){mapObserver.disconnect();currentSector=sector;renderBreadcrumb('sector');const content=$('workspaceContent'),title=document.createElement('div'),focus=document.createElement('section'),grid=document.createElement('div');title.className='workspace-title';title.innerHTML=`<div><h1>Sector ${sector.sector_id}</h1><p>64 physical pages · select a page to enlarge</p></div>`;focus.className='sector-focus';grid.className='sector-focus-grid';grid.setAttribute('role','grid');grid.setAttribute('aria-label',`Sector ${sector.sector_id}, 64 physical pages`);sector.pages.forEach((page,index)=>{const finding=page.diagnostic.state==='known',node=button('',()=>showPage(page.page_id),`page focus-page ${page.allocation}${finding?' finding':''}${page.page_id===selectedPage?' selected':''}`),kind=document.createElement('span'),identity=document.createElement('span');kind.className='page-kind';kind.textContent=page.page_type.state==='known'?page.page_type.value:'not inspected';identity.className='page-id';identity.textContent=String(page.page_id);node.append(kind,identity);node.setAttribute('role','gridcell');node.setAttribute('aria-label',`Page ${page.page_id}, ${page.allocation}${finding?', finding':''}`);node.onkeydown=event=>moveSectorGrid(event,grid,index);grid.append(node)});focus.append(grid);content.replaceChildren(title,focus)}
...
$('closeInfo').addEventListener('click',()=>$('infoDialog').close());$('licenses').addEventListener('click',showLicenses);$('unlockForm').addEventListener('submit',unlock)})();";
