...
// Browser allowance: .scratch/pgbuf-overlay-implementation/browser-memory-budget-revision.md
const browserMemoryBudgetBytes = 64 * 1024 * 1024;
...
test("12288 rendered page cells: per-browser visible latency and paired RSS", async ({ browserName }, testInfo) => {
  ...
  const report = {
    ...
    browserMemoryBudgetBytes,
    p95Ms: p95, incrementalPeakRssBytes: increments, runs,
    timingPass: p95 <= 100, memoryPass: increments.every((increment) => increment <= browserMemoryBudgetBytes),
  };
  ...
  for (const increment of increments) expect(increment).toBeLessThanOrEqual(browserMemoryBudgetBytes);
});
